...
import { ClikeRateServices } from "@/backend/services/ClikeRate";
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Fetch gender data
    const genderData = await ClikeRateServices.getAllClikeData();
...
export async function POST(request: NextRequest) {
  try {
    ...
    const body: IClickRate = await request.json();

    // Create new gender data using the parsed body
    const genderData = await ClikeRateServices.createClikeData(body);
